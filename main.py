...
def train(num_class,aux_loader, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short, alpha, beta, gamma, mu):
    ...
    for i, ((source_data, source_label, source_pos, source_neg),(target_data, target_label, target_pos, target_neg)) in data_loader:
        ...
        loss_classification = criterion(out, label) + 0.001*criterion(out_aux, aux_label)
        ...
        loss = loss_classification + 0.0001*info_nce_loss(feat_source[1], selected_centers, torch.cat([feat_target_neg[1], feat_source_neg[1]],0)) + 0.0001*info_nce_loss(feat_aux[1], feat_aux_pos, feat_aux_neg)#+ 0.1*info_nce_loss(feat_target[1], feat_target_pos[1], feat_target_neg[1])
