...
def main():
	...
	source = 'P01'
	target = 'P08'
	train_source_list = '/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/ZDDA/TranSVAE/dataset/epic-kitchens/list/list_' + source + '_train.txt'
	train_target_list = '/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/ZDDA/TranSVAE/dataset/epic-kitchens/list/list_' +target + '_train.txt' 
	val_list = '/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/ZDDA/TranSVAE/dataset/epic-kitchens/list/list_' + target+'_test.txt'
	...
	ref_lists=['/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/ZDDA/DANN/dataset/ref_'+target+'_'+str(shot)+'shot_split'+str(i)+'.txt' for i in range(20)]
	...
	val_set = TSNDataSet_EpicKitchen("/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/epic_kitchen/features/test/",
						target,
						"/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/ZDDA/TranSVAE/dataset/epic-kitchens/list/list_"+target+"_test.txt",
	...
	source_set = TSNDataSet_EpicKitchen("/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/epic_kitchen/features/train/",
							source,
							"/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/ZDDA/TranSVAE/dataset/epic-kitchens/list/list_"+source+"_train.txt",
	...
	source_sampler = torch.utils.data.sampler.RandomSampler(source_set)
	'''source_class_counts = np.zeros(num_class)#val_set.class_counts()
	for data, label in source_set:
		source_class_counts[label] += 1
	#print(source_class_counts)
	#source_class_counts = source_set.class_counts()
	source_weights = 1.0 / torch.Tensor(source_class_counts)
	source_sampler = WeightedRandomSampler(source_weights, len(source_set))'''

	source_loader = torch.utils.data.DataLoader(source_set, batch_size=args.batch_size[0],sampler=source_sampler,
												num_workers=2, pin_memory=True)
	target_set = TSNDataSet_EpicKitchen("/hkfs/work/workspace_haic/scratch/fy2374-hiwi_workspace/epic_kitchen/features/train/",
							target,
							ref_path,
	...
	#target_sampler = BalancedBatchSampler(target_set, 8, 4)
	'''target_class_counts = np.zeros(num_class)#val_set.class_counts()
	for data, label,_,_ in target_set:
		target_class_counts[label] += 1
	#print(target_class_counts)
	target_weights = 1.0 / torch.Tensor(target_class_counts)
	target_sampler = WeightedRandomSampler(target_weights, len(target_set))'''
...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short, alpha, beta, gamma, mu):
	...
	gms = [[GaussianMixture(n_components=1, random_state=0).fit(np.stack(source_feat_list[j])[:,i,:]) for i in range(8)] for j in range(8)]
	...
	for j in range(num_class):
		...
		for i in range(8):
			...
			base_means = [gms[k][i].means_[0] for k in range(8)]
			base_convs = [gms[k][i].covariances_[0] for k in range(8)]
			num_sampled = 200
	...
	f = open('generated_new_data_D2_D1_Shot10_8.pkl', 'wb')
